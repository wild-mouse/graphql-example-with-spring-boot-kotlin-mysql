...
@RestController
class GraphQLController @Autowired
constructor(private val graphql: GraphQL, private val objectMapper: ObjectMapper) {

    @RequestMapping(value = ["/graphql"], method = [RequestMethod.GET], produces = [MediaType.APPLICATION_JSON_VALUE])
    @CrossOrigin
    @Throws(IOException::class)
    fun graphqlGET(@RequestParam("query") query: String,
                   @RequestParam(value = "operationName", required = false) operationName: String,
                   @RequestParam("variables") variablesJson: String?
    ): Map<String, Any> {
        var variables: Map<String, Any> = LinkedHashMap()
        if (variablesJson != null) {
            variables = objectMapper.readValue(variablesJson, object : TypeReference<Map<String, Any>>() {
    ...
    fun graphql(@RequestBody body: Map<String, Any>): Map<String, Any> {
        ...
        val operationName = if (body["operationName"] != null) body["operationName"] as String else ""
        var variables: Map<String, Any>? = body["variables"] as Map<String, Any>
        if (variables == null) {
